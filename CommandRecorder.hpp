...
public:
    ...
    inline void executeCommands(const CommandHandle * const commandBegin, const CommandHandle * const commandEnd) const noexcept
        { ::vkCmdExecuteCommands(_command, static_cast<std::uint32_t>(std::distance(commandBegin, commandEnd)), commandBegin); }
    ...
    inline void bindDescriptorSets(const PipelineBindPoint pipelineBindPoint, const PipelineLayoutHandle pipelineLayout,
        ...
        const std::uint32_t * const dynamicOffsetBegin = nullptr, const std::uint32_t * const dynamicOffsetEnd = nullptr) const noexcept;

    /** @brief Bind a single descriptor set */
    inline void bindDescriptorSet(const PipelineBindPoint pipelineBindPoint, const PipelineLayoutHandle pipelineLayout,
        const std::uint32_t firstSet, const DescriptorSetHandle descriptorSet,
        const std::uint32_t * const dynamicOffsetBegin = nullptr, const std::uint32_t * const dynamicOffsetEnd = nullptr) const noexcept
    ...
    /** @brief Draw commands */
    inline void draw(const std::uint32_t vertexCount, const std::uint32_t instanceCount = 1,
    ...
    inline void drawIndexed(const std::uint32_t indexCount, const std::uint32_t instanceCount = 1, const std::uint32_t firstIndex = 0,
            const std::int32_t vertexOffset = 0, const std::uint32_t firstInstance = 0u) const noexcept
        { ::vkCmdDrawIndexed(_command, indexCount, instanceCount, firstIndex, vertexOffset, firstInstance); }


    /** @brief Indirect draw commands */
    inline void drawIndirect(const BufferHandle buffer, const BufferSize offset, const std::uint32_t drawCount, const std::uint32_t stride) const noexcept
    ...
    inline void drawIndirectCount(const BufferHandle buffer, const BufferSize bufferOffset,
            const BufferHandle countBuffer, const BufferSize countBufferOffset, const std::uint32_t maxDrawCount, const std::uint32_t stride) const noexcept
    ...
    inline void drawIndexedIndirectCount(const BufferHandle buffer, const BufferSize bufferOffset,
            const BufferHandle countBuffer, const BufferSize countBufferOffset, const std::uint32_t maxDrawCount, const std::uint32_t stride) const noexcept
    ...
    inline void bindVertexBuffers(const std::uint32_t firstBinding, const std::uint32_t bindingCount,
    ...
    inline void bindIndexBuffer(const BufferHandle buffer, const IndexType indexType, const BufferSize offset = 0u) const noexcept
        { ::vkCmdBindIndexBuffer(_command, buffer, offset, static_cast<VkIndexType>(indexType)); }


    /** @brief Buffer copy command with one region */
    inline void copyBuffer(const BufferHandle srcBuffer, const BufferHandle dstBuffer, const BufferCopy &region) const noexcept
    ...
    inline void copyBufferToImage(const BufferHandle srcBuffer, const ImageHandle dstImage, const ImageLayout dstImageLayout, const BufferImageCopy &region) const noexcept
    ...
    inline void copyBufferToImage(const BufferHandle srcBuffer, const ImageHandle dstImage, const ImageLayout dstImageLayout,
        ...
        { ::vkCmdCopyBufferToImage(_command, srcBuffer, dstImage, static_cast<VkImageLayout>(dstImageLayout), static_cast<std::uint32_t>(std::distance(regionBegin, regionEnd)), regionBegin); }


    /** @brief Dispatch a compute shader command */
    inline void dispatch(const std::uint32_t groupCountX, const std::uint32_t groupCountY = 1u, const std::uint32_t groupCountZ = 1u) const noexcept
    ...
    inline void dispatchBase(
            const std::uint32_t groupBaseX, const std::uint32_t groupCountX,
            const std::uint32_t groupBaseY = 0u, const std::uint32_t groupCountY = 1u,
            const std::uint32_t groupBaseZ = 0u, const std::uint32_t groupCountZ = 1u) const noexcept
    ...
    inline void pipelineBarrier(const PipelineStageFlags sourceStage, const PipelineStageFlags destinationStage, const DependencyFlags dependencyFlags,
    ...
    inline void pipelineBarrier(const PipelineStageFlags sourceStage, const PipelineStageFlags destinationStage,
            const DependencyFlags dependencyFlags = DependencyFlags::None) const noexcept
    ...
    inline void pipelineBarrier(const PipelineStageFlags sourceStage, const PipelineStageFlags destinationStage, const DependencyFlags dependencyFlags,
            const MemoryBarrier * const memoryBarrierBegin, const MemoryBarrier * const memoryBarrierEnd) const noexcept
    ...
    inline void pipelineBarrier(const PipelineStageFlags sourceStage, const PipelineStageFlags destinationStage, const DependencyFlags dependencyFlags,
            const MemoryBarrier &memoryBarrier) const noexcept
    ...
    inline void pipelineBarrier(const PipelineStageFlags sourceStage, const PipelineStageFlags destinationStage, const DependencyFlags dependencyFlags,
            const BufferMemoryBarrier * const bufferBarrierBegin, const BufferMemoryBarrier * const bufferBarrierEnd) const noexcept
    ...
    inline void pipelineBarrier(const PipelineStageFlags sourceStage, const PipelineStageFlags destinationStage, const DependencyFlags dependencyFlags,
            const BufferMemoryBarrier &bufferBarrier) const noexcept
    ...
    inline void pipelineBarrier(const PipelineStageFlags sourceStage, const PipelineStageFlags destinationStage, const DependencyFlags dependencyFlags,
            const ImageMemoryBarrier &imageBarrier) const noexcept
